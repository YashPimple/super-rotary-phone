...
const Breakfast = () => {
    ...
    const [input, setInput] = useState('breakfast');
            ...
            <div className="grid gap-4 mt-8 sm:grid-cols-2 lg:grid-cols-4 mx-3 mb-2 " >

                {
                    output.map((item, index) => {
                        return <FoodCard type="Breakfast" name={item.recipe.label} img={item.recipe.image} calories={item.recipe.calories} fat={item.recipe.digest[0].total} carb={item.recipe.digest[1].total} protein={item.recipe.digest[2].total} url={item.recipe.url} key={index}/>
                    })
                }
